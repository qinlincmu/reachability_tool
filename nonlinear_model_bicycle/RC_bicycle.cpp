...
int main(int argc, char *argv[])
{
	...
	int t_id = stateVars.declareVar("t");
        ...
        double phi_0_min = atof(argv[5]);
        double phi_0_max = atof(argv[6]);
        double v_0_min = atof(argv[7]);
        double v_0_max = atof(argv[8]);
        double delta_0_min = atof(argv[9]);
        double delta_0_max = atof(argv[10]);
        double v_noise_min = atof(argv[11]);
        double v_noise_max = atof(argv[12]);
        double delta_noise_min = atof(argv[13]);
        double delta_noise_max = atof(argv[14]);
        
        std::string formula_phi = argv[15];
        double step = atof(argv[16]);

        
	// define the dynamics
	Expression_AST<Real> ode_expression_x("v * cos(phi)");
	Expression_AST<Real> ode_expression_y("v * sin(phi)");
	Expression_AST<Real> ode_expression_phi(formula_phi);
	...
	setting.setFixedStepsize(step, 3);
...
//	setting.setAdaptiveStepsize(0.01, 0.04, 5);
	...
	Interval init_x(x_0_min, x_0_max), init_y(y_0_min, y_0_max), init_phi(phi_0_min, phi_0_max),
