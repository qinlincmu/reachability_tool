...
//	setting.setAdaptiveStepsize(0.01, 0.04, 5);
	...
	Interval init_x(-0.01, 0.01), init_y(-0.01, 0.01), init_psi(-0.01, 0.01), init_beta(-0.00466938241262, 0.00466938241262), init_v(-0.01, 0.01), init_t;
	...
	vector<Constraint> unsafeSet1;
        vector<Constraint> unsafeSet2;

        Constraint cons1("x-y");
        Constraint cons2("3*x+(-4*y)");
        unsafeSet1.push_back(cons1);
        unsafeSet2.push_back(cons2);
        //cout << formula << endl;
	...
	dynamics.reach(result, setting, initialSet, unsafeSet);
        cout<<result.num_of_flowpipes<<endl;
	...
	unsigned int total_size = result.safety_of_flowpipes.size();
        ...
        {
            cout<<"+++"<<*safetyIter<<endl;
            //fp = *fpIter;
            int safety1 = (*fpIter).safetyChecking(unsafeSet1, setting.tm_setting, setting.g_setting);
            cout<<"property 1 "<<safety1<<endl;
            int safety2 = (*fpIter).safetyChecking(unsafeSet2, setting.tm_setting, setting.g_setting);
            cout<<"property 2 "<<safety2<<endl;
        } 
